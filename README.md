...
本項目執行指令 ```demo.bat 2```，主要[原始碼](./src/demo2.sh)

+ 建立共管群組與帳號
    - 群組 ```mgtdata```
    - 用戶 ```tuser1```、```tuser2```、```tuser3```
    - 僅 ```tuser1```、```tuser2``` 屬於 ```mgtdata``` 群組

+  建立共管目錄
    - 建立目錄於 ```/var/data```
    - 目錄群組透過 ```chgrp``` 命令更換為 ```mgtdata```
    - 目錄操作權限透過 ```chmod``` 更換為擁有用戶與群組可以讀寫執行，但其他用戶不可動作

+ 切換帳戶測試
    - 執行 ```touch``` 指令來產生一個檔案
    - 除了 ```tuser3``` 外，另外兩個用戶可正常增加
    - ```tuser3``` 自設定上無法寫入也無法讀取該目錄

### 應用程式限定用戶

本項目執行指令 ```demo.bat 3```，主要[原始碼](./src/demo3.sh)

+ 建立應用程式指令 ```app1```
    - 建立一般用戶，並不設定密碼，但建立家目錄
    - 於家目錄建立一個 ```app.sh```
    - 在 ```/usr/bin``` 目錄建立連結至 ```app.sh```

+ 建立共用應用程式指令 ```app2```
    - 於 ```/usr/local``` 建立一個 ```app.sh```
    - 在 ```/usr/bin``` 目錄建立連結至 ```app.sh```

+ 切換帳戶測試
    - ```app1``` 除了 root 與 appuser 外皆無法執行，因為其應用程式腳本所在位置無法進入導致無法正確找到命令
    - ```app2``` 則是所有人皆可以執行

### 應用程式共管目錄

本項目執行指令 ```demo.bat 4```，主要[原始碼](./src/demo4.sh)

+ 建立應用程式指令 ```app1```，參考前個範例，但修改輸出並寫入目標 ```/var/data/raw``` 檔案

+ 建立共用應用程式指令 ```app2```，參考前個範例，但修改輸出並寫入目標 ```/var/data/raw``` 檔案

+  建立共管群組與目錄
    - 群組 ```mgtdata```，並給於 appuser 此群組
    - 建立目錄 ```/var/data``` 與 ```/var/data/raw``` 檔案
    - 目錄群組透過 ```chgrp``` 命令更換為 ```mgtdata```
    - 目錄操作權限透過 ```chmod``` 更換為擁有用戶與群組可以讀寫執行，但其他用戶僅讀執行

+ 切換帳戶測試
    - 用 appuser 執行 ```app1```
    - 用 root 執行 ```app2```
    - 用 testuser 執行 ```tail``` 來顯示內容
    - 測試結果，root、appuser 皆可寫入至目標檔案，且可以透過第三方帳號讀取該檔案內容
